...
public class TrackManager : MonoBehaviour
{
    ...
    public IEnumerator Begin()
    {
        if (!m_Rerun)
        {
            ...
            // Initialize the system (not on rerun)
            ...
            string characterName = "Cat";//PlayerData.instance.characters[PlayerData.instance.usedCharacter];
            GameObject characterPrefab = Resources.Load<GameObject>("Characters/Cat");

            if (characterPrefab == null)
            {
                Debug.LogWarning($"Unable to load character {characterName} from Resources.");
                yield break;
            }

            // ✅ Instantiate the player character
            GameObject playerObject = Instantiate(characterPrefab, Vector3.zero, Quaternion.identity);
            Character player = playerObject.GetComponent<Character>();

            if (player == null)
            {
                Debug.LogWarning($"Character {characterName} does not have a Character component.");
                yield break;
            }
            ...
            // ✅ Parent player character to the character collider
            ...
            // ✅ Load theme
            m_CurrentThemeData = m_IsTutorial ? tutorialThemeData : ThemeDatabase.GetThemeData(PlayerData.instance.themes[PlayerData.instance.usedTheme]);
...
//"level" in our game are milestone the player have to reach : one every 300m
    ...
    public IEnumerator SpawnNewSegment()
    {
        ...
        // ✅ Select a random segment from the current zone's prefab list
        int segmentUse = Random.Range(0, m_CurrentThemeData.zones[m_CurrentZone].prefabListObj.Length);
        if (segmentUse == m_PreviousSegment)
            segmentUse = (segmentUse + 1) % m_CurrentThemeData.zones[m_CurrentZone].prefabListObj.Length;

        // ✅ Load segment prefab from Resources
        string segmentName = m_CurrentThemeData.zones[m_CurrentZone].prefabListObj[segmentUse].name;
        GameObject segmentPrefab = Resources.Load<GameObject>("Themes/Default/TrackSegments/" + segmentName);

        if (segmentPrefab == null)
        {
            Debug.LogWarning($"Unable to load segment {segmentName} from Resources.");
            yield break;
        }

        // ✅ Instantiate segment
        GameObject segmentObject = Instantiate(segmentPrefab, _offScreenSpawnPos, Quaternion.identity);
    ...
    public void SpawnObstacle(TrackSegment segment)
    {
        if (segment.possibleObstacles.Length != 0)
        {
            ...
            {
                string assetRef = segment.refObstacles[Random.Range(0, segment.possibleObstacles.Length)].name;
    ...
    private IEnumerator SpawnFromAssetReference(string reference, TrackSegment segment, int posIndex)
    {
       

        Debug.Log("obstacles :: " + reference);
        GameObject obj = Resources.Load<GameObject>("Themes/Default/Obstacles/" + reference);
        if (obj != null)
        {
                ...
                if (laneValid)
                {
                    ...
                    if (Random.value < powerupChance)
                    {
                        ...
                        if (consumableDatabase.consumbales[picked].canBeSpawned)
                        {
                            ...
                            // ✅ Load powerup prefab from Resources
                            GameObject powerupPrefab = Resources.Load<GameObject>("Powerup/" + consumableDatabase.consumbales[picked].gameObject.name);

                            if (powerupPrefab == null)
                            {
                                Debug.LogWarning($"Unable to load consumable {consumableDatabase.consumbales[picked].gameObject.name} from Resources.");
                                yield break;
                            }

                            // ✅ Instantiate powerup
                            toUse = Instantiate(powerupPrefab, pos, rot);
                    ...
                    else if (Random.value < premiumChance)
                    {
                        ...
                        // ✅ Load premium collectible from Resources
                        GameObject premiumPrefab = Resources.Load<GameObject>("PremiumCollectible/" + currentTheme.premiumCollectible.name);

                        if (premiumPrefab == null)
                        {
                            Debug.LogWarning($"Unable to load collectible {currentTheme.premiumCollectible.name} from Resources.");
                            yield break;
                        }

                        // ✅ Instantiate premium collectible
                        toUse = Instantiate(premiumPrefab, pos, rot);
                        toUse.transform.SetParent(segment.transform, true);
                    }
                    else
                    {
                        // ✅ Use pooled coins instead of loading from Resources
